...
class CreateInv:
    ...
    def __submit_familyjob(self, vendor):
        ...
        FAMILY_ENDPOINT = "{}/api/extras/scripts/probe_address.ProbeDevice/".format(self.__NETBOX_URL)
        payload = {
            "data": {
    ...
    #        yaml.dump(py_obj,f,sort_keys=False, default_style='"') 
    ...
    def __convert_ips(self, obj):
        if isinstance(obj, dict):
            ...
            return str(obj)  # Ensure IPs are treated as strings, without extra quotes
        return obj
    ...
    def writeInventory(self):
        if self.product != 'all':
            ...
            self.__write_yaml_to_file(data,self.__fam_device)
        else:   
            data = {}
            for item in self.__family_items:
                families=item[family]
                self.__iplist = self.__retrieveIpList()
                ...
                for ip in self.__iplist:
                    ...
                    if check == True:
                        ...
                        data[self.family_name][families]['hosts'][hostname]['ansible_host']='{}'.format(ip)
            self.__write_yaml_to_file(data,families)
               # print(f"{key}: {value}")

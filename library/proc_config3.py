...
def run_pexpect(commands, options, host, ssh_user, ssh_pass, time):
    ...
    i=child.expect (['Continue', 'Store key', 'password:', 'Access granted', 'Press any key', '#'], timeout=10)
    if i == 0:
        child.sendline('y')
    elif i == 1:
        child.sendline('n')
        ...
        child.sendline('\r')
    j=child.expect (['Continue', 'Store key', 'password:', 'Access granted', 'Press any key', '#'], timeout=10)
    if j == 0:
        child.sendline('y')
    elif j == 1:
        child.sendline('n')
    ...
    if commands:
        ...
        for command in commands:#for line in ipfile:
            ...
            #sleep(2)
            #i=child.expect ([pexpect.TIMEOUT, pexpect.EOF], timeout=2)
            i=child.expect(['#', 'continue', pexpect.TIMEOUT, pexpect.EOF], timeout=120)
            if i == 0:
                current_settings = child.before
                print (child.before)
                if child.before:
    ...
    #child.sendline('boot system flash secondary')
    #hostname = child.after
    #logfile = "/home/max/ansible/unibo/filelog"
    #child.sendline('boot system flash primary')
    ##hostname = child.after

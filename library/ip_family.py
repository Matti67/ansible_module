...
family_name = input("Select Family macro type id to retrieve json product list ")
# Define the script endpoint and payload
#SCRIPT_ENDPOINT = f"{NETBOX_URL}/api/extras/scripts/probe_devrole.DeviceRole/"
SCRIPT_ENDPOINT = "{}/api/extras/scripts/prob_device_id.ProbeDeviceId/".format(NETBOX_URL)
payload = {
    "data": {
...
try:
    ...
    #iplist =[]
    #####print(f"this is the job_complete: {job_complete}")
    addrlist = []
    indexs = []
    familys =[]
    ...
    ##    #print(f"{index}")
    ##    family = line.rsplit(',',1)[0]
    ##    #print(f"{family}")
    ##    indexs.append(index)
    ##    familys.append(family)
    #print(f"familys list: {familys}")
    #print(f"indexs list: {indexs}")
    #create a list with just real ip address
    #for i in addrlist:
    #    if str(i[:1])=='1':
    #        iplist.append(i)
    ##without index and family
    ##for i, j in zip(indexs, familys):
    ##    family_list.append({i: j})
    ##adding index and family
    for line in job_complete.split('\n'):
    ...
    procurve_items = [item for item in family_list if item["family"].startswith("{}".format(family_name))]
    print(json.dumps(procurve_items, indent=4))
